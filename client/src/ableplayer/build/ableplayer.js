...
(function ($) {
  /**
   * Function that starts building able-player
   * @param options:Object - settings for the able-player
   * {
   *  element: HTMLElement - video/audio
   *  onOpenTranscript: Function - callback, which will be called right before the transcript is opened
   *  onCloseTranscript: Function - callback, which will be called right before the transcript is closed
   * CANCELED onOpenCaption: Function - callback, which will be called right before the caption is opened
   * CANCELED onCloseCaption: Function - callback, which will be called right before the caption is closed
   *  onPlayButtonClick: Function - callback, which will be called when the play button is clicked
   * }
   */
  window.startAblePlayer = function(options){
    try{
      if (!options.element) {
        throw Error('Element is not defined');
      }

      AblePlayer.prototype.callbacks = {
        onRefreshControls: options.onRefreshControls,
        onResizeWindow: options.onResizeWindow,
        onCreatePlayer: options.onCreatePlayer,
        onOpenCaption: options.onOpenCaption,
        onCloseCaption: options.onCloseCaption,
        onOpenTranscript: options.onOpenTranscript,
        onCloseTranscript: options.onCloseTranscript,
        onPlayButtonClick: options.onPlayButtonClick,
        onMediaStarted: options.onMediaStarted,
        onMediaPaused: options.onMediaPaused,
        onMediaCompleted: options.onMediaCompleted
      };

      new AblePlayer($(options.element));
...
(function ($) {
  ...
  AblePlayer.prototype.refreshControls = function() {
    ...
    if (this.seekBar) {
      ...
      if (this.isFullscreen()) {
        ...
        /**
         * Changed this.$ableWrapper.width() => this.$ableDiv.width();
         */
        seekbarWidth = this.$ableDiv.width() - widthUsed - seekbarSpacer;
      }
    ...
    if (this.callbacks && this.callbacks.onRefreshControls) {
      this.callbacks.onRefreshControls(this);
    }
  };
  ...
  AblePlayer.prototype.handlePlay = function(e) {
    ...
    /**
     * The callback function that calls when the play button is clicked
     */
    if (this.callbacks && this.callbacks.onPlayButtonClick){
      this.callbacks.onPlayButtonClick(this);
    }
  };
  ...
  */
  ...
  AblePlayer.prototype.handleTranscriptToggle = function () {
    if (this.$transcriptDiv.is(':visible')) {
      ...
      /**
       * The callback is called before right before the transcript is closed
       */
      if (this.callbacks && this.callbacks.onCloseTranscript) {
        this.callbacks.onCloseTranscript(this);
      }
    }
    else {
      ...
      /**
       * The callback is called before right before the transcript is opened
       */
      if (this.callbacks && this.callbacks.onOpenTranscript) {
        this.callbacks.onOpenTranscript(this);
      }
    }
...
(function ($) {
  ...
  AblePlayer.prototype.injectTranscriptArea = function() {
	...
	// Hadley update  - do not want draggable
    if (!this.transcriptDivLocation) {
...
(function ($) {
  ...
  AblePlayer.prototype.onMediaPause = function () {
    /**
     * Calls callback when the media is paused
     */
    if (this.callbacks && this.callbacks.onMediaPaused){
      this.callbacks.onMediaPaused(this);
    }
  };
  ...
  AblePlayer.prototype.onMediaComplete = function () {
     ...
     * Calls callback when the media is paused
     */
    if (this.callbacks && this.callbacks.onMediaCompleted){
      this.callbacks.onMediaCompleted(this);
    }
  };
  ...
  AblePlayer.prototype.onWindowResize = function () {
    ...
    /**
     * Calls callback when the window is resized
     */
    if (this.callbacks && this.callbacks.onResizeWindow){
      this.callbacks.onResizeWindow(this);
    }
  };
  ...
  AblePlayer.prototype.onPlayerKeyPress = function (e) {
    ...
    if (which === 27) {
      ...
      if (!$(document.activeElement).is(function(i, e){if (e.tagName == 'TEXTAREA' || e.tagName == 'BUTTON' || e.tagName == 'A' || e.tagName == 'INPUT') {return false;}return true;
      })) {
        return false;
      }
